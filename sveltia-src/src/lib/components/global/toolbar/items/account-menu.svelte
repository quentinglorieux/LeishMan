<script>
  ...
  const signedInLabel = $derived.by(() => {
    if (isLocalRepo) return $_('working_with_local_repo');
    if (isTestRepo) return $_('working_with_test_repo');

    const base = $_('signed_in_as_x', { values: { name: $user?.login } });

    return $user?.orcid ? `${base} (${$user.orcid})` : base;
  });
</script>
